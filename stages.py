...
def execute(engine, YOURTOKEN, season):
    ...
    r = requests.get(f'https://api.sportmonks.com/v3/football/stages/seasons/{season}?api_token={YOURTOKEN}')
    j = r.json()
    cabecalho = pd.DataFrame()
    cabecalho = pd.DataFrame.from_dict(j['data'])
    cabecalho = cabecalho.reset_index()
    df_stages = cabecalho
    df_stages.to_sql('stages', engine, schema = 'sportMonx', if_exists='append', index=False)
